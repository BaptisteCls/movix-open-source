...
const ESTIMATED_BITRATE_TOOLTIP =
  "Le bitrate est estimé à partir de la taille du fichier (content-length) et de la durée du flux. C'est une valeur approximative.";
...
const HLSPlayer = forwardRef<HLSPlayerRef, HLSPlayerProps>(({
  ...
  const [externalLanguages] = useState<Array<{ code: string; label: string }>>([
    ...
    { code: 'srp', label: 'Српски' },
    { code: 'slv', label: 'Slovenščina' },
    { code: 'est', label: 'Eesti' },
    { code: 'lav', label: 'Latviešu' },
    { code: 'lit', label: 'Lietuvių' },
  ...
  const formatDetectedStreamQuality = useCallback((level?: {
    ...
    const bitrate = bitrateCandidates.find((value): value is number => Number.isFinite(value) && value > 0) || null;
  ...
  const sourceGroups = useMemo(() => {
    ...
    if (darkinoSources && darkinoSources.length > 0) {
      hlsSources.push({
        type: 'darkino_main',
        id: 'darkino_main',
        label: `🔥 Nightflix (${darkinoSources.length})`,
        url: '#',
      });
    }
    ...
    if (canPlayBravo && purstreamSources && purstreamSources.length > 0) {
      hlsSources.push({
        type: 'bravo_main',
        id: 'bravo_main',
        label: `🎬 Bravo (${purstreamSources.length})`,
        url: '#',
      });
    }
    ...
    if (customSources && customSources.length > 0) {
      customSources.forEach((source, index) => {
        ...
        embedSources.push({
          type: 'custom',
          id: `custom_${index}`,
          label: isSeek ? `🌟 SeekStreaming ${index + 1}` : `🌟 Lecteur Movix ${index + 1}`,
          url: source,
        });
      });
    }
    ...
    if (omegaSources && omegaSources.length > 0) {
      embedSources.push({
        type: 'omega_main',
        id: 'omega_main',
        label: `🎬 Lecteurs Omega (${omegaSources.length})`,
        url: '#',
      });
    }
    ...
    if (coflixSources && coflixSources.length > 0) {
      embedSources.push({
        type: 'multi_main',
        id: 'multi_main',
        label: `🎬 Lecteurs Multi (${coflixSources.length})`,
        url: '#',
      });
    }
    ...
    if (fstreamSources && fstreamSources.length > 0) {
      ...
      embedSources.push({
        type: 'fstream_main',
        id: 'fstream_main',
        label: `🎬 Lecteurs FStream (${fstreamSources.length})`,
        url: '#',
      });
    } else {
    ...
    if (wiflixSources && wiflixSources.length > 0) {
      ...
      embedSources.push({
        type: 'wiflix_main',
        id: 'wiflix_main',
        label: `🦁 Lecteurs Lynx (${wiflixSources.length})`,
        url: '#',
      });
    } else {
    ...
    if (viperSources && viperSources.length > 0) {
      ...
      embedSources.push({
        type: 'viper_main',
        id: 'viper_main',
        label: `🐍 Lecteurs Viper (${viperSources.length})`,
        url: '#',
      });
    }
    ...
    if (voxSources && voxSources.length > 0) {
      ...
      embedSources.push({
        type: 'vox_main',
        id: 'vox_main',
        label: `🟢 Lecteurs Vox (${voxSources.length})`,
        url: '#',
      });
    }
    ...
    if (embedUrl && embedType === 'adfree') {
      embedSources.push({
        type: 'adfree',
        id: 'adfree_main',
        label: '✅ Lecteur Lumedia (sans pub)',
        url: embedUrl,
        isActive: true
      });
    }
  ...
  // Helper function to extract m3u8 URL from a source URL
  const extractM3u8Url = async (sourceUrl: string): Promise<string> => {
    // This is a simple implementation - in a real-world scenario,
    // this might involve making a request to the source URL and extracting the m3u8 from the response
    if (sourceUrl.includes('.m3u8')) {
      return sourceUrl; // Already an m3u8 URL
    }
    // For our purposes, we'll just return the original URL if it doesn't contain m3u8
    return sourceUrl;
  };
  ...
  // Amélioration de la gestion des contrôles sur mobile
  const handleMobileTouchStart = () => {
    if (isMobile) {
      // On ne fait rien ici, le toggle est géré dans handleVideoClick
      if (controlsTimeoutRef.current) {
        clearTimeout(controlsTimeoutRef.current);
        controlsTimeoutRef.current = undefined;
      }
  ...
  // Double-clic : ne fullscreen que si pas sur un bouton, contrôle, poster, etc.
  const handleDoubleClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Appeler toggleFullscreen sur mobile ET desktop
    toggleFullscreen();
  };
                                  ...
                                  {[
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <span className="text-xs text-gray-400">VO/VOSTFR</span>
  ...
  return (
    ...
    >
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 sm:gap-6 max-w-3xl w-full pt-16 sm:pt-20">
            {poster && (
              <div className="w-56 h-80 md:w-64 md:h-96 rounded-xl overflow-hidden shadow-2xl border border-white/10 relative z-10">
                <img src={getImageUrl(poster, 'w780')} alt="Poster" className="w-full h-full object-cover" />
              </div>
            )}
      ...
      {zoomState.isZoomed && (
        ...
        >
          <span className="text-lg font-bold">Zoom {Math.round(zoomState.scale * 100)}%</span>
          <button
            onClick={resetZoom}
              ...
              {(airPlayAvailable || castAvailable) && (
                <div className="relative flex items-center h-[24px]">
                  <button
                    ...
                    className="text-white hover:text-red-600 transition-colors flex items-center justify-center"
                    aria-label={airPlayAvailable && !castAvailable ? "AirPlay" : castAvailable && !airPlayAvailable ? "Cast" : t('watch.streamTo')}
                  >
                    <motion.div
                      animate={{
                                  ...
                                  >
                                    <div>
                                      ...
                                      <div className="text-xs text-gray-400">
                                        {(externalLanguages.find(l => l.code === sub.SubLanguageID)?.label || sub.LanguageName || sub.SubLanguageID) || 'Unknown'} • {sub.SubFormat || 'srt'}
                                        {tvShowId && seasonNumber && episodeNumber && (
                  ...
                  >
                    ...
                    <div className="mb-3">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-sm text-gray-300">{subtitleStyle.fontSize.toFixed(1)} rem</span>
                      </div>
                      <input
                        type="range"
                        min="0.5"
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-blue-400">Informations du flux</span>
              </div>
            ...
            <div className="space-y-1.5">
              {videoRef.current && (
                <>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>{videoRef.current.duration ? `${Math.floor(videoRef.current.duration / 3600).toString().padStart(2, '0')}:${Math.floor((videoRef.current.duration % 3600) / 60).toString().padStart(2, '0')}:${Math.floor(videoRef.current.duration % 60).toString().padStart(2, '0')}` : 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>{Math.round(volume * 100)}%{videoRef.current.muted ? ' (muet)' : ''}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">État</span>
                    <span>{videoRef.current.paused ? '⏸ Pause' : '▶ Lecture'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Boucle</span>
                    <span>{isLooping ? 'Activée' : 'Désactivée'}</span>
                  </div>
                  <div className="flex justify-between">
              ...
              {hlsRef.current && (
                <>
                  ...
                  <div className="text-gray-400 text-[10px] uppercase tracking-wider mb-1">HLS</div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    <span className="text-gray-400">Pistes audio</span>
                    <span>{audioTracks.length}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Sous-titres</span>
                    <span>{currentSubtitle === 'off' ? 'Désactivés' : currentSubtitle}</span>
                  </div>
                </>
              )}
              {!hlsRef.current && (
              ...
              <div className="flex justify-between">
                ...
                <span>{isFullscreen ? 'Oui' : 'Non'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">PiP</span>
                <span>{isPipActive ? 'Actif' : 'Inactif'}</span>
              </div>
            </div>
            <div className="mt-3 text-[10px] text-gray-500 text-center">Ctrl+J pour fermer</div>
          </motion.div>
        )}
      </AnimatePresence>
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                  <span className="text-xl">⌨️</span> Raccourcis clavier
                </h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1 text-sm">
                {/* Lecture */}
                <div className="col-span-1 sm:col-span-2 text-xs font-bold text-red-400 uppercase tracking-wider mt-2 mb-1">Lecture</div>
                {[
                  ...
                  ['End', 'Fin de la vidéo'],
                  ['1-9', 'Aller à 10%-90%'],
                  ['R', 'Activer/désactiver la boucle'],
                ].map(([key, desc]) => (
                ...
                {/* Volume */}
                <div className="col-span-1 sm:col-span-2 text-xs font-bold text-red-400 uppercase tracking-wider mt-3 mb-1">Volume</div>
                {[
                  ['↑ / ↓', 'Volume +/- 5%'],
                ...
                {/* Vitesse */}
                <div className="col-span-1 sm:col-span-2 text-xs font-bold text-red-400 uppercase tracking-wider mt-3 mb-1">Vitesse</div>
                {[
                  ['Shift + > / <', 'Accélérer / Ralentir (±0.25x)'],
                  ['+ / -', 'Accélérer / Ralentir (±0.25x)'],
                  ['0', 'Vitesse normale (1x)'],
                  ['. (pause)', 'Image suivante'],
                  [', (pause)', 'Image précédente'],
                ].map(([key, desc]) => (
                  <div key={key} className="flex justify-between py-1">
                    <kbd className="bg-gray-800 px-2 py-0.5 rounded text-xs text-gray-300 font-mono border border-gray-700">{key}</kbd>
                    <span className="text-gray-400 text-xs">{desc}</span>
                ...
                {/* Affichage */}
                <div className="col-span-1 sm:col-span-2 text-xs font-bold text-red-400 uppercase tracking-wider mt-3 mb-1">Affichage</div>
                {[
                  ['F', 'Plein écran'],
                  ['P', 'Picture-in-Picture'],
                  ['C', 'Cycler les sous-titres'],
                  ['Ctrl + J', 'Infos du flux vidéo'],
                  ['Shift + ?', 'Cette aide'],
                  ['Échap', 'Fermer les overlays'],
                ].map(([key, desc]) => (
                  <div key={key} className="flex justify-between py-1">
                    <kbd className="bg-gray-800 px-2 py-0.5 rounded text-xs text-gray-300 font-mono border border-gray-700">{key}</kbd>
                    <span className="text-gray-400 text-xs">{desc}</span>
              ...
              <div className="mt-5 text-center text-xs text-gray-500">
                Appuyez sur <kbd className="bg-gray-800 px-1.5 py-0.5 rounded text-gray-400 border border-gray-700">Échap</kbd> ou <kbd className="bg-gray-800 px-1.5 py-0.5 rounded text-gray-400 border border-gray-700">Shift + ?</kbd> pour fermer
              </div>
            </div>
          </motion.div>
        )}
...
// Dropdown component for external languages
    ...
    <div className="relative" ref={ref}>
      ...
      >
        <span className="flex items-center gap-2">
          ...
          {selectedLang ? selectedLang.label : 'Choisir la langue'}
        </span>
      ...
      {open && (
        <ul className="absolute z-50 mt-1 w-full bg-gray-900 border border-gray-700 rounded shadow-lg max-h-60 overflow-auto" role="listbox">
          {languages.map(lang => (
            <li
              key={lang.code}
              className={`flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-gray-800 ${selected === lang.code ? 'bg-red-600 text-white' : 'text-gray-200'}`}
